...
Term_offset=$(echo "(-8.0/3.0)*((${t}*${t})/(${U}-3.0*${J})) +  ((-4.0/3.0)*((${t}*${t})/${U}))" | bc -l)
Term_LL=$( echo "( (4.0/3.0)*( (${t}*${t})/(${U}-3*${J}) ) )  +  (  (-1.0/3.0)*( (${t}*${t})/(${U}) )  )"  | bc -l )
Term_LL2=$( echo "( (4.0/3.0)*( (${t}*${t})/(${U}-3*${J}) ) )  +  (  (1.0/6.0)*( (${t}*${t})/(${U}) )  )"  | bc -l )
...
Term_SL=$( echo "(  (${lambda}/2.0)  - ((${lambda}*${lambda})/(8.0*${J})) -  ( (7.0*${lambda}*${lambda})/(40.0*${J}) )   )" | bc -l  )
...
sed -i "s/Term_SL/${Term_SL}/g" J1file_L${Length}_use.txt

sed -i "s/Term_LL2/${Term_LL2}/g" J2file_L${Length}_use.txt
sed -i "s/Term_SSLL/${Term_SSLL}/g" J2file_L${Length}_use.txt

sed -i "s/Term_SSLL2/${Term_SSLL2}/g" J3file_L${Length}_use.txt
