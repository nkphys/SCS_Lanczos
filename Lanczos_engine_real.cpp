...
void LANCZOS::Perform_LANCZOS(Matrix_COO &Hamil){
    ...
    double temp1_type_double, temp3_type_double, temp2_type_double, temp4_type_double;
    int Target_state=0;
    Mat_1_doub B2 ,Norms;
    ...
    double tmpnrm;
    double tmpnrm_type_double2, tmpnrm2;
        ...
        if(lanc_iter!=0){
            if(to_reorth==true){
                ...
                for(int i=0;i<Krylov_space_vecs.size();i++){
                    temp_dot[i]=dot_product(Krylov_space_vecs[i],Kvector_np1);
                }
...
//                }
    ...
    if(Dynamics_performed==false){
        ...
        for(int Ts=0;Ts<states_to_look.size();Ts++){
            ...
            double norm_ev=dot_product(Eig_vecs[Ts], Eig_vecs[Ts]);
            for(int j=0;j<Hamil.nrows;j++){
        ...
        if(get_overlap_with_basis==true){
            ...
            for(int Ts=0;Ts<Eig_vecs.size();Ts++){
                ...
                for(int bi=0;bi<Eig_vecs[0].size();bi++){
                    ...
                    Overlaps[Ts][bi].first=Eig_vecs[Ts][bi]*Eig_vecs[Ts][bi];
    ...
    */
    ...
    for(int opr_no=0;opr_no<one_point_obs.size();opr_no++){
        ...
        for(int site=0;site<T_sites;site++){
            ...
            Matrix_COO_vector_multiplication("U", One_point_oprts[opr_no][site], Eig_vecs[state_no], temp_vec);
...
void LANCZOS::Measure_two_point_observables(Mat_1_string two_point_obs, Hamiltonian_3_COO & Two_point_oprts,
                                            int T_sites, Mat_1_doub vec_, bool PBC_check){

    cout<<"Two-point Measurement is started after completing Lanczos algorithm for arbitary state "<<endl;
    //Krylov_space_vecs


    double norm_ =  dot_product(vec_,vec_);
        ...
        for(int site=0;site<T_sites;site++){
            for(int site2=0;site2<T_sites;site2++){
                ...
                if(site2>=site){
                    ...
                    Matrix_COO_vector_multiplication(mult_type, Two_point_oprts[opr_no][site][site2], vec_, temp_vec);


                    value = dot_product(vec_,temp_vec);
...
void LANCZOS::Measure_two_point_observables(Mat_1_string two_point_obs, Hamiltonian_3_COO &Two_point_oprts, int T_sites,  int state_no, bool PBC_check){
    ...
    cout<<"Two-point Measurement is started after completing Lanczos algorithm fo state_no = "<<states_to_look[state_no]<<endl;
    //Krylov_space_vecs
            ...
            for(int site2=0;site2<T_sites;site2++){
                ...
                if(site2>=site){
                    ...
                    for(int i=0;i<Krylov_space_vecs.size();i++){
                        ...
                        Matrix_COO_vector_multiplication(mult_type, Two_point_oprts[opr_no][site][site2], Krylov_space_vecs[i], temp_vec);
...
void LANCZOS::Get_Dynamics_seed(Matrix_COO &Opr){
    ...
    double tmpnrm_type_double=(dot_product(Dynamics_seed,Dynamics_seed));

    //cout<<"tmpnrm_type_double = "<<tmpnrm_type_double<<endl;
