...
void MODEL_3_orb_Hubb_chain::Add_diagonal_terms(BASIS_3_orb_Hubb_chain &basis){
    ...
    for (int i=0;i<basis.D_up_basis.size();i++){
        for (int j=0;j<basis.D_dn_basis.size();j++){
            ...
            value+=U*countCommonBits(basis.D_up_basis[i],basis.D_dn_basis[j])*one;
...
void MODEL_3_orb_Hubb_chain::Initialize_macro_oprs_to_calculate(BASIS_3_orb_Hubb_chain &basis){
    ...
    for (int i=0;i<basis.D_up_basis.size();i++){
        for (int j=0;j<basis.D_dn_basis.size();j++){
            ...
            if(value2!=0){
                Macro_oprts[1].value.push_back(value2*one);
            ...
            if(value3!=0){
                Macro_oprts[2].value.push_back(value3*one);
            ...
            if(value4!=0){
                Macro_oprts[5].value.push_back(value4*one);
                ...
                for(int gamma_p=gamma+1;gamma_p<3;gamma_p++){
                    for(int site=0;site<basis.Length;site++){
                                ...
                                )
                        {
                            ...
                            Macro_oprts[3].value.push_back(sign_FM*(0.5*(-J_H*2.0))*one);
                                ...
                                )
                        {
                            ...
                            assert(m_new<m);
                            Macro_oprts[4].value.push_back(sign_FM*J_H*one);
                        ...
                        for(int gamma_p=gamma;gamma_p>=0;gamma_p--){
                            ...
                            { // nearest neighbour
                                        ...
                                        )
                                {
                                    ...
                                    if((Hopping_mat_NN[gamma_p][gamma])!=0){
                                        assert(m_new<m);
                                        Macro_oprts[5].value.push_back(-1.0*sign_FM*(Hopping_mat_NN[gamma_p][gamma])*one);
