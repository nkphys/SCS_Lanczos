...
*/
#ifndef USE_COMPLEX
#include "Model_3_orb_Hubbard_chain_two_SzSectors.h"
#include <stdlib.h>
using namespace std;
...
3)  For total
    m=(basis.D_dn_basis_sec1.size()*i1 + j1)*basis_size_sec2
       +
...
void MODEL_3_orb_Hubb_chain_two_SzSectors::Add_diagonal_terms(BASIS_3_orb_Hubb_chain_two_SzSectors &basis){
    ...
    Hamil.nrows = basis.basis_size_sec1 + basis.basis_size_sec2;
    ...
    for(int sec=0;sec<2;sec++){
        if(sec==0){
            D_up_basis=basis.D_up_basis_sec1;
            D_dn_basis=basis.D_dn_basis_sec1;
            offset=0;
        }
        else{
            D_up_basis=basis.D_up_basis_sec2;
            D_dn_basis=basis.D_dn_basis_sec2;
            offset=basis.basis_size_sec1;
        }

        for (int i=0;i<D_up_basis.size();i++){
            for (int j=0;j<D_dn_basis.size();j++){
                m=offset + D_dn_basis.size()*i + j;

                value=0;
                //intra-orbital coulomb repulsion:
                value+=U*countCommonBits(D_up_basis[i],D_dn_basis[j]);
                ...
                for(int gamma=0;gamma<3;gamma++){
                    for(int gamma_p=gamma+1;gamma_p<3;gamma_p++){
                        for(int site=0;site<basis.Length;site++){
                            value+=(U_p - (J_H*0.5))*
                                    ( ( bit_value(D_up_basis[i],gamma*basis.Length + site) +
                                        bit_value(D_dn_basis[j],gamma*basis.Length + site) )*
                                      ( bit_value(D_up_basis[i],gamma_p*basis.Length + site) +
                                        bit_value(D_dn_basis[j],gamma_p*basis.Length + site) )
                                      );
                        }
                    }
                }
...
void MODEL_3_orb_Hubb_chain_two_SzSectors::Add_non_diagonal_terms(BASIS_3_orb_Hubb_chain_two_SzSectors &basis){
    ...
    for(int sec=0;sec<2;sec++){
        if(sec==0){
            D_up_basis=basis.D_up_basis_sec1;
            D_dn_basis=basis.D_dn_basis_sec1;
            inverse_Dup=basis.inverse_Dup_sec1;
            inverse_Ddn=basis.inverse_Ddn_sec1;
            offset=0;
        }
        else{
            D_up_basis=basis.D_up_basis_sec2;
            D_dn_basis=basis.D_dn_basis_sec2;
            inverse_Dup=basis.inverse_Dup_sec2;
            inverse_Ddn=basis.inverse_Ddn_sec2;
            offset=basis.basis_size_sec1;
        }
        for (int i=0;i<D_up_basis.size();i++){
            for (int j=0;j<D_dn_basis.size();j++){
                ...
                for(int gamma=0;gamma<3;gamma++){
                    for(int gamma_p=gamma+1;gamma_p<3;gamma_p++){
                        for(int site=0;site<basis.Length;site++){
                            ...
                            if(
                                    ((bit_value(D_dn_basis[j],gamma*basis.Length + site)==1)
                                     &&
                                     (bit_value(D_up_basis[i],gamma*basis.Length + site)==0)
                                     )
                                    &&
                                    ((bit_value(D_up_basis[i],gamma_p*basis.Length + site)==1)
                                     &&
                                     (bit_value(D_dn_basis[j],gamma_p*basis.Length + site)==0)
                                     )
                                    )
                            {

                                D_up = (int) (D_up_basis[i] - pow(2,gamma_p*basis.Length + site)
                                              + pow(2,gamma*basis.Length + site) );
                                D_dn = (int) (D_dn_basis[j] + pow(2,gamma_p*basis.Length + site)
                                              - pow(2,gamma*basis.Length + site) );

                                i_new = inverse_Dup[D_up - D_up_basis[0]];
                                j_new = inverse_Ddn[D_dn - D_dn_basis[0]];

                                m_new = offset + D_dn_basis.size()*i_new + j_new;

                                l=gamma*basis.Length + site;
                                lp=gamma_p*basis.Length + site;

                                sign_pow_up = one_bits_in_bw(l,lp,D_up_basis[i]);
                                sign_pow_dn = one_bits_in_bw(l,lp,D_dn_basis[j]);
                            ...
                            if(
                                    ((bit_value(D_dn_basis[j],gamma*basis.Length + site)==0)
                                     &&
                                     (bit_value(D_up_basis[i],gamma*basis.Length + site)==0)
                                     )
                                    &&
                                    ((bit_value(D_up_basis[i],gamma_p*basis.Length + site)==1)
                                     &&
                                     (bit_value(D_dn_basis[j],gamma_p*basis.Length + site)==1)
                                     )
...
void MODEL_3_orb_Hubb_chain_two_SzSectors::Add_connections(BASIS_3_orb_Hubb_chain_two_SzSectors &basis){
                        ...
                        for(int site_p=0;site_p<basis.Length ;site_p++){
                            ...
                            for(int gamma_p=gamma;gamma_p>=0;gamma_p--){
                                ...
                                { // nearest neighbour
                                    ...
                                    if(
                                            (bit_value(D_up_basis[i],gamma*basis.Length + site)==1)
                                            &&
                                            (bit_value(D_up_basis[i],gamma_p*basis.Length + site_p)==0)
                                            )
                                    {

                                        D_up = (int) (D_up_basis[i] + pow(2,gamma_p*basis.Length + site_p)
                                                      - pow(2,gamma*basis.Length + site) );
                                    ...
                                    if(
                                            (bit_value(D_dn_basis[j],gamma*basis.Length + site)==1)
                                            &&
                                            (bit_value(D_dn_basis[j],gamma_p*basis.Length + site_p)==0)
                                            )
                                    {

                                        D_dn = (int) (D_dn_basis[j] + pow(2,gamma_p*basis.Length + site_p)
                                                      - pow(2,gamma*basis.Length + site) );
...
void MODEL_3_orb_Hubb_chain_two_SzSectors::Read_parameters(BASIS_3_orb_Hubb_chain_two_SzSectors &basis, string filename){
